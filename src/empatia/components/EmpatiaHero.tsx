...
interface Props {
  data: EmpatiaProposal
}

export function EmpatiaHero({ data }: Props) {
  ...
  return (
    <header className="proposal-gradient relative overflow-hidden px-4 pb-24 pt-16 sm:px-6 lg:px-8">
      ...
      />
      <motion.div
        className="pointer-events-none absolute bottom-0 left-0 h-64 w-64 rounded-full bg-accent-500/15 blur-3xl"
        animate={{ x: [0, 30, 0] }}
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="mb-8 flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          <motion.img
            ...
            className="h-20 w-auto rounded-lg bg-white/10 p-2 shadow-lg sm:h-24"
          />
          <motion.span
            initial={{ opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            className="inline-flex items-center gap-2 rounded-full border border-brand-400/30 bg-brand-500/10 px-4 py-1.5 text-sm font-medium text-brand-200"
          >
            {hero.badge}
          </motion.span>
        </div>
